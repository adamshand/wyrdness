...
For the first pass, implement the pragmatic grouping over the existing legacy channels. Once stable, improve the detector layer while keeping the same public visual vocabulary:

```js
visual.parallel = max(raw.correlated_high, raw.correlated_low);
visual.antiparallel = max(raw.anti_ab, raw.anti_ba, raw.walk_separate ?? 0);
visual.stick_together = raw.stick; // preferably augmented by random-walk closeness
visual.pearson = raw.pearson;
```

The anomaly ring is **not** a fifth colour/channel. It is WyrdWeb's safe Stage 3 threshold event marker and replaces the undesirable web strobe behaviour.
...
- `anti_ba`
- `stick`
- `walk_separate` if implemented as a Channel 3max-like amber contributor
- `pearson`
...
4. Update demo sequence to cycle through visual channels, followed by the safe Stage 3 anomaly ring showcase:

```js
['parallel', 'antiparallel', 'stick_together', 'pearson', 'anomaly']
```

The anomaly segment demonstrates the high-significance threshold effect, not a separate statistical channel.
...
- Demo may include the final anomaly ring/stage-3 segment.
...
Visuals may drift from Wyrd's exact device presentation for web safety and UX, but the statistical pattern detection should match the public Wyrd model as closely as reasonable.

Priorities after the visual refactor is stable:
...
2. Channel 3max approximation. **Implemented as first-pass walk-separation detector.**
   - Wyrd: longer-term/mean maximum distance between streams.
   - Amber strength is now `max(Channel 2 approximation, walk_separate approximation)`.
   - Current implementation uses random-walk path-distance ratios against a Brownian/random-walk expectation; further work should empirically calibrate p-values.

3. Replace bitwise `stick` detector. **Implemented as first-pass walk-closeness detector.**
   - Wyrd Channel 3min appears closer to random-walk path closeness / minimum mean distance than simple bit-by-bit equality.
   - Blue/stick now uses random-walk closeness/path-distance rather than bitwise agreement.
   - Bitwise agreement remains visible in debug as `Agree z` only, not as the primary blue strength.
...
- [ ] Demo cycles through the four visual channels plus the safe anomaly-ring showcase.
